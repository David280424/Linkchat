...
object ChatManager {
    ...
    fun sendMediaMessage(receiverUid: String, uri: Uri, mediaType: String, onResult: (ok: Boolean, error: String?) -> Unit) {
        val meUid = auth.currentUser?.uid ?: return
        val chatId = chatIdWith(receiverUid)
        val now = System.currentTimeMillis()
        val extension = if (mediaType == "image") "jpg" else "aac"
        val ref = storage.reference.child("chat_media/$chatId/$now.$extension")

        ref.putFile(uri).addOnSuccessListener {
            ref.downloadUrl.addOnSuccessListener { downloadUrl ->
                val previewText = if(mediaType == "image") "📷 Foto" else "🎤 Audio"
                val data = mapOf(
                    "senderId" to meUid,
                    "receiverId" to receiverUid,
                    "text" to previewText,
    ...
    // ACTUALIZADO: Añadido callType para distinguir entre voz y video
    fun startCallSignal(otherUid: String, channelName: String, callType: String = "video") {
        val chatId = chatIdWith(otherUid)
    ...
    fun endCallSignal(otherUid: String) {
        val chatId = chatIdWith(otherUid)
        db.collection("chats").document(chatId).update("activeCall", FieldValue.delete())
    }
